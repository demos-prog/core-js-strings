...
function removeFirstOccurrences(str, value) {
  ...
  const resultStr = str.slice(0, index) + str.slice(index + value.length);
  return resultStr;
}
...
function formatTime(minutes, seconds) {
  const propMinutes = minutes.toString().padStart(2, '0');
  const propSeconds = seconds.toString().padStart(2, '0');
  return `${propMinutes}:${propSeconds}`;
}
